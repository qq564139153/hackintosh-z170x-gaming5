...
class SMBIOS:
    ...
    def select_smbios_model(self, hardware_report, macos_version):
        ...
        if platform != "Laptop" and list(hardware_report.get("GPU").items())[-1][-1].get("Device Type") != "Integrated GPU":
            # Prefer CPU-matched iMac SMBIOS with native Polaris AGPM when possible.
            # iMacPro1,1 uses Vega power tables and often causes UI stutter on RX 570/580.
            discrete_gpu = list(hardware_report.get("GPU").items())[-1][-1]
            gpu_codename = discrete_gpu.get("Codename", "")
            if gpu_codename in ("Polaris 20", "Polaris 22", "Polaris 10", "Polaris 11", "Polaris 12", "Baffin", "Ellesmere"):
                if "Kaby Lake" in codename or "Amber Lake" in codename:
                    return "iMac18,3"
                if "Coffee Lake" in codename or "Comet Lake" in codename or "Whiskey Lake" in codename or "Cannon Lake" in codename:
                    if self.utils.parse_darwin_version(macos_version) < self.utils.parse_darwin_version("18.0.0"):
                        return "iMac18,3"
                    return "iMac19,1"
                if "Skylake" in codename and self.utils.parse_darwin_version(macos_version) < self.utils.parse_darwin_version("22.0.0"):
                    return "iMac17,1"
            return smbios_model
