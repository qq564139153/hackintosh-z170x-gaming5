        ...
        if kexts[kext_data.kext_index_by_name.get("WhateverGreen")].checked:
            for gpu_name, gpu_info in hardware_report.get("GPU", {}).items():
                if gpu_info.get("Device Type") == "Integrated GPU":
                    ...
                    spoofed_id = pci_data.SpoofGPUIDs.get(gpu_info.get("Device ID"))
                    spoofed_device = spoofed_id.split("-")[-1].lower()
                    # Prefer a clean marketing name for spoofed Polaris cards
                    spoof_model = gpu_name
                    if spoofed_id == "1002-67FF":
                        spoof_model = "AMD Radeon RX 570"
                    elif spoofed_id == "1002-67DF":
                        spoof_model = "AMD Radeon RX 580"

                    add_device_property(gpu_info.get("PCI Path"), {
                        "device-id": self.utils.to_little_endian_hex(spoofed_device),
                        "model": spoof_model
                    })
